...
bool Calibrator::extractCorners() {
  ...
  if (fnL.size() != fnR.size() || fnL.empty()) {
    std::cerr << "Error: Different number of images or empty folder."
              << std::endl;
    return false;
  }
  ...
  for (size_t i = 0; i < fnL.size(); i++) {
    ...
    bool foundL = cv::findChessboardCorners(
        grayL, boardSize, cornersL,
        cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_NORMALIZE_IMAGE);
    bool foundR = cv::findChessboardCorners(
        grayR, boardSize, cornersR,
        cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_NORMALIZE_IMAGE);
...
void Calibrator::compute(cv::Mat& K1, cv::Mat& D1, cv::Mat& K2, cv::Mat& D2,
                         cv::Mat& R, cv::Mat& T, cv::Size& imgSize) {
  if (objectPoints.size() < 10)
    std::cerr << "Warning: Not enough valid pairs for good calibration."
              << std::endl;
  ...
  double rms1 = cv::calibrateCamera(objectPoints, imgPointsL, imageSize, K1, D1,
                                    rvecs, tvecs);
  double rms2 = cv::calibrateCamera(objectPoints, imgPointsR, imageSize, K2, D2,
                                    rvecs, tvecs);
  ...
  double rms = cv::stereoCalibrate(
      objectPoints, imgPointsL, imgPointsR, K1, D1, K2, D2, imageSize, R, T, E,
      F, flags,
